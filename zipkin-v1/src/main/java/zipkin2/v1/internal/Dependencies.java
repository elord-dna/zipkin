...
public final class Dependencies {
  ...
  void write(Buffer buffer) {
    ...
    writeLong(buffer, startTs);

    END_TS.write(buffer);
    writeLong(buffer, endTs);
  ...
  static final class DependencyLinkAdapter implements Buffer.Writer<DependencyLink> {
    ...
    public void write(DependencyLink value, Buffer buffer) {
      PARENT.write(buffer);
      writeLengthPrefixed(buffer, value.parent());

      CHILD.write(buffer);
      writeLengthPrefixed(buffer, value.child());

      CALL_COUNT.write(buffer);
      writeLong(buffer, value.callCount());

      if (value.errorCount() > 0) {
        ERROR_COUNT.write(buffer);
        writeLong(buffer, value.errorCount());
      }
